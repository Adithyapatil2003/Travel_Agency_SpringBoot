		...
		if (packages.size() != 0) {
			...
			if (customerDetails.size() != 0) {
				...
				for (CustomerDetails customerDetail : customerDetails) {
					...
					AddressDto permanentAddressDto = new AddressDto(customerDetail.getPermanentAdd().getHouseNumber(),
							customerDetail.getPermanentAdd().getStreet(),
							customerDetail.getPermanentAdd().getLandmark(), customerDetail.getPermanentAdd().getCity(),
							customerDetail.getPermanentAdd().getState(), customerDetail.getPermanentAdd().getPinCode());

					AddressDto communicationAddressDto = new AddressDto(
							customerDetail.getCommunicationAdd().getHouseNumber(),
							customerDetail.getCommunicationAdd().getStreet(),
							customerDetail.getCommunicationAdd().getLandmark(),
							customerDetail.getCommunicationAdd().getCity(),
							customerDetail.getCommunicationAdd().getState(),
							customerDetail.getCommunicationAdd().getPinCode());

					TourDetailsDto tourDetailsDto = new TourDetailsDto(customerDetail.getTourDetails().getId(),
							customerDetail.getTourDetails().getStartLocation(),
							customerDetail.getTourDetails().getCost(),
							customerDetail.getTourDetails().getDestLocation(),
							customerDetail.getTourDetails().getLocationsCovered(),
							customerDetail.getTourDetails().getPackageName());
				...
				throw new CustomerNotFoundException("No customer is registered with " + packageName);
			}
		} else {
...
//------------------------------------------------------------------------------------------------------------------------//
	...
	public TourDetailsDto getTourDesc(String packageName) {
		List<TourDetails> tourDetails = tourDetailsRepository.findByPackageName(packageName);

		if (tourDetails.size()!=0) {

			TourDetails tourDetail=tourDetails.getFirst();
			TourDetailsDto tourDetailsDto = new TourDetailsDto(tourDetail.getId(), tourDetail.getStartLocation(),
					tourDetail.getCost(), tourDetail.getDestLocation(), tourDetail.getLocationsCovered(),
					tourDetail.getPackageName());
...
//--------------------------------------------------------------------------------------------------------------------//
	...
	public TourDetailsDto addTour(TourDetailsDto tourDetailsDto) {
		...
		TourDetails tourDetails = new TourDetails(0, tourDetailsDto.getStartLocation(),
